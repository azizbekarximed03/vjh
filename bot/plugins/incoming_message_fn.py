...
async def incoming_start_message_f(bot, update):
    ...
    await bot.send_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton('Kanalim', url='https://t.me/Azizbek_03')
                ],
    ...
    try:
      if int(update.command[1]) <= 90 and int(update.command[1]) >= 10:
        ...
        try:
          await bot.send_message(
            chat_id=update.chat.id,
            text="10'dan 90'gacha bo‘lgan raqam yozing",
            reply_to_message_id=update.message_id
          )
          return
        except:
...
async def incoming_cancel_message_f(bot, update):
  ...
  if os.path.exists(status):
    ...
    await update.reply_text("Ishonchingiz komilmi? 🚫 Bu hozir to‘xtaydi!", reply_markup=reply_markup, quote=True)
  else:
    delete_downloads()
    await bot.send_message(
      chat_id=update.chat.id,
